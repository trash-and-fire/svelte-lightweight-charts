...
<Chart {...options} localization={localization}>
    <AreaSeries
        data={data}
        ...
        lineWidth={2}
    />
    <Switcher list={AVAILABLE_FORMATTERS} bind:value={selected}/>
</Chart>
...
<script>
    ...
    const FORMATTERS = {
        ...
        Dollar: (price) => '$' + price.toFixed(2),
        // eslint-disable-next-line @typescript-eslint/typedef
        Pound: (price) => '\u00A3' + price.toFixed(2),
    };
    const AVAILABLE_FORMATTERS = Object.keys(FORMATTERS);

    let selected = AVAILABLE_FORMATTERS[0];
    let localization;
    $: localization = {
        priceFormatter: FORMATTERS[selected],
    };
